                ...
                <div class="relative bg-white rounded-[2.5rem] shadow-[0_20px_70px_-15px_rgba(15,23,42,0.12)] border border-slate-100 p-2 lg:p-3 flex flex-col lg:flex-row items-stretch lg:items-center gap-2">
                    ...
                    <div class="flex-1 relative group px-6 py-2 lg:py-0 border-b lg:border-b-0 lg:border-r border-slate-50">
                        ...
                        <form action="{{ url()->current() }}" method="GET" class="w-full flex items-center">
                            @if(request('mitra')) <input type="hidden" name="mitra" value="{{ request('mitra') }}"> @endif
                    ...
                    <div class="flex-1 relative group px-6 py-2 lg:py-0">
                        ...
                        <form action="{{ url()->current() }}" method="GET" class="w-full flex items-center">
                            @if(request('kota')) <input type="hidden" name="kota" value="{{ request('kota') }}"> @endif
                            <select name="mitra" id="mitra"
                                ...
                                @if(isset($daftarMitra))
                                    @foreach($daftarMitra as $mtr)
                                        <option value="{{ $mtr->id }}" {{ request('mitra') == $mtr->id ? 'selected' : '' }}>{{ $mtr->nama_rental }}</option>
                                    @endforeach
                                @endif
                            </select>
                            <div class="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-slate-300">
                        ...
                        @if(request('mitra'))
                            <a href="{{ request()->fullUrlWithQuery(['mitra' => null]) }}"
                               class="absolute -top-3 right-6 px-3 py-1 bg-red-50 text-red-600 rounded-full text-[10px] font-black tracking-widest uppercase border border-red-100 shadow-sm hover:bg-red-100 transition-colors">
                                <i class="fa-solid fa-rotate-left mr-1"></i> Reset
                            </a>
                        @endif
...
<div class="group bg-white rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col">
    ...
    {{-- AREA GAMBAR: background biru muda, gambar transparan --}}
    <div class="relative m-3 rounded-2xl overflow-hidden h-52"
        ...
        {{-- Kotak putih transparan di dalam --}}
        ...
        <img src="{{ $mobil->image_url }}"
             ...
             class="w-full h-full object-contain relative z-10 pt-2 pb-2 scale-[1.15] group-hover:scale-[1.2] transition-transform duration-500 mix-blend-multiply {{ $mobil->status != 'tersedia' ? 'grayscale opacity-60' : '' }}">
...
@empty
            ...
            @if($mobils instanceof \Illuminate\Pagination\LengthAwarePaginator && $mobils->hasPages())
