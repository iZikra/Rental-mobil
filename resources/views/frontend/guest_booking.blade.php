...
<body class="bg-slate-50 text-slate-800 font-sans antialiased selection:bg-blue-500 selection:text-white pb-10">
    ...
    <div class="relative bg-slate-900 overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <header class="flex items-center justify-between mb-8">
                <a href="{{ route('home') }}" class="flex items-center space-x-4 bg-white/10 hover:bg-white/20 transition px-4 py-2 rounded-2xl backdrop-blur-sm border border-white/10">
    ...
    <main class="relative -mt-10 px-4 sm:px-6 lg:px-8 z-10 max-w-7xl mx-auto">
        ...
        <form action="{{ route('guest.booking.submit', ['token' => $transaksi->booking_token]) }}" method="POST" enctype="multipart/form-data" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div class="flex items-center bg-white border border-gray-300 rounded-xl px-4 py-3 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition shadow-sm hover:border-blue-300">
                                    ...
                                    <input type="text" name="nama_customer" id="nama_customer" value="{{ old('nama_customer') }}" required class="bg-transparent border-none w-full text-slate-800 font-semibold focus:ring-0 placeholder-gray-400" placeholder="Sesuai KTP Anda">
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="flex items-center bg-white border border-gray-300 rounded-xl px-4 py-3 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition shadow-sm hover:border-blue-300">
                                    ...
                                    <input type="text" name="telp_customer" id="telp_customer" value="{{ old('telp_customer') }}" required inputmode="numeric" class="bg-transparent border-none w-full text-slate-800 font-semibold focus:ring-0 placeholder-gray-400" placeholder="08123xxxx">
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-2">
                            <div class="space-y-4">
                                ...
                                <div class="flex gap-2">
                                    <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ old('tanggal_mulai', $transaksi->tgl_ambil ?? date('Y-m-d')) }}" min="{{ date('Y-m-d') }}" class="w-3/5 bg-slate-50 border border-slate-200 focus:border-blue-500 rounded-xl px-4 py-3 font-semibold text-slate-800 transition" required>
                                    <input type="time" name="jam_mulai" id="jam_mulai" value="{{ old('jam_mulai', '09:00') }}" class="w-2/5 bg-slate-50 border border-slate-200 focus:border-blue-500 rounded-xl px-2 py-3 font-semibold text-slate-800 text-center transition" required>
                                </div>
                            </div>
                            <div class="space-y-4">
                                <label class="block text-xs font-bold text-gray-500 uppercase">Perkiraan Selesai <span class="text-red-500">*</span></label>
                                <div class="flex gap-2">
                                    <input type="date" name="tanggal_selesai" id="tanggal_selesai" value="{{ old('tanggal_selesai', $transaksi->tgl_kembali ?? date('Y-m-d', strtotime('+1 day'))) }}" class="w-3/5 bg-slate-50 border border-slate-200 focus:border-blue-500 rounded-xl px-4 py-3 font-semibold text-slate-800 transition" required>
                                    <input type="time" name="jam_selesai" id="jam_selesai" value="{{ old('jam_selesai', '09:00') }}" class="w-2/5 bg-slate-50 border border-slate-200 focus:border-blue-500 rounded-xl px-2 py-3 font-semibold text-slate-800 text-center transition" required>
                                </div>
                            </div>
                        </div>
                ...
                <div class="rounded-3xl border border-slate-200/70 bg-white shadow-[0_18px_60px_-30px_rgba(15,23,42,0.35)] overflow-hidden">
                    ...
                    <div class="px-6 sm:px-8 py-6">
                        <div class="bg-slate-50 border border-slate-200 rounded-2xl p-5 h-48 overflow-y-auto mb-5 text-sm text-slate-600 space-y-3 shadow-inner">
                            <h4 class="font-bold text-gray-800">Kebijakan Mitra Rental ({{ $car->rental->nama_rental ?? 'Mitra DriveNow' }}):</h4>
                        ...
                        <div class="p-6">
                            ...
                            <div class="mb-5 text-center">
                                ...
                                <p class="text-sm text-gray-500 font-semibold mt-1">
                                    {{ $car->branch->nama_cabang ?? $car->rental->nama_rental ?? 'Mitra DriveNow' }} • {{ $car->branch->kota ?? 'Pusat' }}
                                </p>
    ...
    <footer class="mt-16 py-8 border-t border-gray-200 text-center">
        <div class="mx-auto flex justify-center items-center gap-1.5 opacity-40 text-sm font-bold text-slate-900">
            <i class="fa-solid fa-car-side"></i> <span>DriveNow {{ date('Y') }} ©️ Hak Cipta Dilindungi</span>
        </div>
    </footer>
