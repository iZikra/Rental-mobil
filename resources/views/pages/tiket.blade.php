...
<head>
    ...
    <title>E-Tiket #{{ $transaksi->id }} - {{ config('app.name', 'Rental Mobil') }}</title>
...
<body class="py-10">
    ...
    <div class="max-w-xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden relative">
        ...
        {{-- === TAMPILAN WATERMARK MIRING === --}}
        <div class="absolute inset-0 flex items-center justify-center pointer-events-none z-10 overflow-hidden">
            <span class="transform -rotate-45 border-4 {{ $classWatermark }} opacity-20 text-6xl font-black px-10 py-4 uppercase tracking-widest rounded-xl whitespace-nowrap">
                {{ $textWatermark }}
            </span>
        </div>
        {{-- =================================== --}}
        ...
        <div class="bg-tokopedia p-4 text-white flex justify-between items-center no-print-bg relative z-20">
            ...
            <div class="text-sm opacity-90">{{ date('d F Y, H:i') }}</div>
        </div>
        ...
        <div class="p-6 relative z-20">
            ...
            <div class="flex justify-between items-start mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Kode Booking: <span class="text-tokopedia font-bold font-mono">INV/{{ date('Ymd') }}/RENT/{{ $transaksi->id }}</span></p>
                </div>
                <div class="text-right">
                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/84/Tokopedia_2019_logo.svg/1200px-Tokopedia_2019_logo.svg.png" alt="Logo" class="h-8 opacity-0 hidden"> {{-- Placeholder Logo --}}
                    <span class="block text-xs font-bold text-gray-400 uppercase tracking-wide">Penyewa</span>
                    <span class="block font-bold text-gray-800">{{ $transaksi->nama }}</span>
                </div>
            </div>
            ...
            <div class="flex gap-4 mb-4">
                ...
                <div class="w-24 h-24 flex-shrink-0 bg-gray-100 rounded-md overflow-hidden border border-gray-200">
                    @if($transaksi->mobil && $transaksi->mobil->gambar)
                        <img src="{{ asset('storage/'.$transaksi->mobil->gambar) }}" class="w-full h-full object-cover">
                    @else
                ...
                {{-- Info Mobil --}}
                <div class="flex-1">
                    <h2 class="font-bold text-lg text-gray-800">{{ $transaksi->mobil->merk }} {{ $transaksi->mobil->model }}</h2>
                    <p class="text-sm text-gray-500 mb-2">{{ $transaksi->mobil->nopol }} • {{ $transaksi->mobil->tahun }}</p>
                    
                    <div class="flex flex-wrap gap-2">
                        <span class="bg-green-50 text-tokopedia text-xs px-2 py-1 rounded font-bold border border-green-100">
                            {{ $transaksi->sopir == 'dengan_sopir' ? 'Dengan Sopir' : 'Lepas Kunci' }}
                        </span>
                        <span class="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded">
                            {{ $transaksi->lama_sewa }} Hari
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-sm text-gray-600">
                    <span>Harga Sewa ({{ $transaksi->lama_sewa }} hari)</span>
                    <span>Rp {{ number_format($transaksi->total_harga - ($transaksi->sopir == 'dengan_sopir' ? 150000 * $transaksi->lama_sewa : 0), 0, ',', '.') }}</span>
                </div>
                
                @if($transaksi->sopir == 'dengan_sopir')
                <div class="flex justify-between text-sm text-gray-600">
                    <span>Jasa Sopir</span>
                    <span>Rp {{ number_format(150000 * $transaksi->lama_sewa, 0, ',', '.') }}</span>
                </div>
                @endif
                ...
                <div class="border-t-2 border-dashed border-gray-200 my-2 pt-2 flex justify-between items-center">
                    ...
                    <span class="font-bold text-xl text-orange-500">Rp {{ number_format($transaksi->total_harga, 0, ',', '.') }}</span>
                </div>
            </div>
