        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-12 relative">
                ...
                @if(Auth::check() && Auth::user()->role == 'admin')
                    <div class="mt-6">
                        <a href="{{ route('admin.tentang_kami.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150 shadow-lg transform hover:scale-105">
                            + Tambah Seksi Baru
                        </a>
                    </div>
                @endif
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg hover:shadow-md transition-shadow duration-300 relative group">
                        ...
                        @if(Auth::check() && Auth::user()->role == 'admin')
                            <div class="absolute top-4 right-4 flex space-x-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                                <a href="{{ route('admin.tentang_kami.edit', $item->id) }}" class="bg-yellow-400 text-white p-2 rounded hover:bg-yellow-500 shadow-sm" title="Edit">
                                    ✏️
                                </a>
                                
                                <form action="{{ route('admin.tentang_kami.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus seksi ini?');">
                                    @csrf @method('DELETE')
