        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-blue-500 to-blue-700 rounded-3xl p-5 shadow-xl shadow-blue-100 text-white transform hover:scale-[1.02] transition duration-300">
                    ...
                    <h3 class="text-3xl font-black mt-1.5">{{ count($pesanan) }} <span class="text-xs font-semibold text-blue-200">Pesanan</span></h3>
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-amber-500 to-amber-700 rounded-3xl p-5 shadow-xl shadow-amber-100 text-white transform hover:scale-[1.02] transition duration-300">
                    ...
                    <h3 class="text-3xl font-black mt-1.5">{{ $pesanan->where('status', 'Pending')->count() }} <span class="text-xs font-semibold text-amber-200">Baru</span></h3>
                    <div class="mt-3 flex items-center gap-1.5 text-[10px] text-amber-100/80">
                        @if($pesanan->where('status', 'Pending')->count() > 0)
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-3xl p-5 shadow-xl shadow-indigo-100 text-white transform hover:scale-[1.02] transition duration-300">
                    ...
                    <h3 class="text-3xl font-black mt-1.5">{{ $pesanan->where('status', 'Disewa')->count() }} <span class="text-xs font-semibold text-indigo-200">Aktif</span></h3>
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-3xl p-5 shadow-xl shadow-emerald-100 text-white transform hover:scale-[1.02] transition duration-300">
                    ...
                    <h3 class="text-3xl font-black mt-1.5">{{ $pesanan->where('status', 'Selesai')->count() }} <span class="text-xs font-semibold text-emerald-200">Transaksi</span></h3>
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        @if($p->mobil)
                                            <div class="text-sm font-bold text-gray-900 uppercase tracking-tight flex items-center gap-2">
                                                <span>{{ $p->mobil->merk ?? 'UNIT' }} {{ $p->mobil->model ?? 'UNKNOWN' }}</span>
                                                @if($p->mobil->no_plat)
                                            ...
                                            <div class="flex flex-col gap-1.5 mt-2">
                                                ...
                                                {{-- Tambahan Informasi Penting (Sopir, Tujuan, Catatan) --}}
                                                <div class="flex flex-wrap gap-1.5 mt-1">
                                                    @if($p->sopir == 'dengan_sopir')
                                                    ...
                                                    @if($p->tujuan)
                                                        <span class="text-[9px] bg-amber-50 text-amber-700 px-2 py-0.5 rounded border border-amber-100 font-extrabold uppercase flex items-center gap-1 shadow-sm" title="Tujuan: {{ $p->tujuan }}">
                                                            📍 Ke: {{ \Illuminate\Support\Str::limit($p->tujuan, 15) }}
                                                        </span>
                                                    @endif

                                                    @if($p->catatan)
                                                        <span class="text-[9px] bg-purple-50 text-purple-700 px-2 py-0.5 rounded border border-purple-100 font-extrabold uppercase flex items-center gap-1 shadow-sm" title="Catatan: {{ $p->catatan }}">
                                                            📝 {{ \Illuminate\Support\Str::limit($p->catatan, 15) }}
                                                        </span>
                                                    @endif
                                                </div>
                                            </div>
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap text-center">
                                        ...
                                        @if($p->biaya_tambahan > 0)
                                            <div class="text-[8px] text-rose-500 font-extrabold uppercase mt-1" title="Termasuk biaya tambahan layanan antar-jemput luar kantor">
                                                + Rp {{ number_format($p->biaya_tambahan, 0, ',', '.') }} Layanan
                                            </div>
                                        @endif
                                    </td>
