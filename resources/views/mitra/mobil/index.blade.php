        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-3xl p-6 shadow-xl shadow-indigo-100 text-white transform hover:scale-[1.02] transition duration-300">
                    ...
                    <h3 class="text-4xl font-black mt-2">{{ count($mobils) }} <span class="text-sm font-medium text-indigo-200">Unit</span></h3>
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-3xl p-6 shadow-xl shadow-emerald-100 text-white transform hover:scale-[1.02] transition duration-300">
                    ...
                    <h3 class="text-4xl font-black mt-2">{{ $mobils->where('status', 'tersedia')->count() }} <span class="text-sm font-medium text-emerald-200">Unit</span></h3>
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-amber-500 to-amber-700 rounded-3xl p-6 shadow-xl shadow-amber-100 text-white transform hover:scale-[1.02] transition duration-300">
                    ...
                    <h3 class="text-4xl font-black mt-2">{{ $mobils->where('status', '!=', 'tersedia')->count() }} <span class="text-sm font-medium text-amber-200">Unit</span></h3>
                                ...
                                <tr class="hover:bg-slate-50/70 transition duration-200 group">
                                    ...
                                    <td class="px-6 py-5 w-40">
                                        <div class="relative overflow-hidden rounded-2xl bg-slate-50 border border-slate-150 p-2 h-24 flex items-center justify-center shadow-inner group-hover:border-blue-300 group-hover:bg-blue-50/20 transition-all duration-300">
                                            <img src="{{ $m->image_url }}"
                                                 alt="{{ $m->merk }} {{ $m->model }}"
                                                 class="max-h-full max-w-full object-contain relative z-10 group-hover:scale-110 transition-transform duration-500 filter drop-shadow-md {{ $m->status != 'tersedia' ? 'grayscale opacity-60' : '' }}"
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex flex-col">
                                            <div class="flex items-center gap-2">
                                                <span class="text-lg font-black text-slate-800 uppercase tracking-tight">{{ $m->model }}</span>
                                                @if($m->no_plat)
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        <span class="inline-flex items-center gap-1 bg-indigo-50 border border-indigo-150 text-indigo-700 text-[10px] font-black uppercase px-2.5 py-1 rounded-full shadow-inner">
                                            ...
                                            {{ $m->branch->nama_cabang ?? 'Pusat' }} ({{ $m->branch->kota ?? 'Medan' }})
                                        </span>
                                    </td>
                                    
                                    {{-- Harga Sewa --}}
                                    <td class="px-6 py-5 text-center whitespace-nowrap">
                                        <div class="text-base font-black text-slate-900">Rp {{ number_format($m->harga_sewa, 0, ',', '.') }}</div>
                                    ...
                                    <td class="px-6 py-5 text-center whitespace-nowrap">
                                        <div class="flex justify-center gap-2">
                                            <a href="{{ route('mitra.mobil.edit', $m->id) }}" class="group/btn bg-amber-50 hover:bg-amber-500 text-amber-600 hover:text-white p-2.5 rounded-xl border border-amber-200 hover:border-amber-500 shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5" title="Edit Armada">
