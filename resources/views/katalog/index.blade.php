...
<div class="container mx-auto p-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-8 border border-gray-200">
        <form action="{{ route('katalog.index') }}" method="GET" class="flex flex-col md:flex-row items-center gap-4 justify-center">
        ...
        @forelse($mobils as $mobil)
            <div class="bg-white border rounded-lg p-5 shadow-lg relative overflow-hidden">
                ...
                <p class="text-blue-600 font-extrabold text-xl mb-4">
                    Rp {{ number_format($mobil->harga_sewa, 0, ',', '.') }} <span class="text-sm text-gray-500 font-normal">/ hari</span>
                </p>
                
                <hr class="my-4 border-gray-300">
                
                <p class="text-md mb-1">📍 Lokasi Cabang: <strong class="text-red-600">{{ $mobil->branch->kota }}</strong></p>
                <p class="text-md mb-6">🏢 Mitra Rental: <strong>{{ $mobil->rental->nama_rental }}</strong></p>
                
                <button onclick="alert('Fitur Booking menuju mobil ID: {{ $mobil->id }}')" class="block w-full text-center bg-blue-600 text-white font-bold py-3 rounded hover:bg-blue-800 transition">
                    Pilih Mobil Ini
                </button>
            </div>
        @empty
            <div class="col-span-3 bg-red-100 text-red-700 text-center p-6 rounded-lg font-bold border border-red-300">
                ⚠️ Maaf, saat ini tidak ada unit mobil yang tersedia di lokasi "{{ request('kota') }}".
            </div>
        @endforelse
    </div>
